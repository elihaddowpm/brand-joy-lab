...
3. **Rests on enough people.** `aud_n >= AUDIENCE_MIN_N` (**75**). Below that, the centered gap is too noisy to trust — the DB function hard-filters these out before they reach you, so any row in `audience_affinity` already clears this test.

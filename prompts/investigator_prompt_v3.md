...
- **`bjl_audience_affinity_v2(home_items)`** — the audience defined by preference for the home experience; returns what else that audience distinctively over-prefers across the numeric constructs, centered within construct. Return columns: `construct`, `primary_topic`, `item_name`, `rel_lift`, `audience_score`, `general_score`, `aud_n`, `audience_size`, `reportable`, `audience_thin`. Fire on any audience-shaped question — first turn or follow-up (see the trigger phrases above). Do not wait for a follow-up to run the arm on a first-turn audience question.
  ...
  - **The thin band, and why a zero-row result used to be unreadable.** `aud_n` counts people drawn FROM the audience, so it can never exceed the audience itself. When the whole audience is smaller than `min_aud_n`, no item can clear the bar and the arm returns nothing — the request was arithmetically impossible, not answered. Every returned row now carries `audience_thin`. When the audience is below `min_aud_n` the per-item floor relaxes to 30 and every row comes back `audience_thin: true`; keep those rows and pass them through, and the synthesizer will attach the thin-base warning. `min_aud_n` itself is unchanged — 75 is still the bar for an unqualified finding.
  - **A zero-row audience arm is a question, not an answer. Run `bjl_audience_size` before you conclude anything.** Zero rows has three completely different causes that look identical, and only one of them is a real finding:

    ```sql
    SELECT * FROM bjl_audience_size(ARRAY['<the same home items you just passed>']);
    -- returns: names_given, names_resolved, respondents_scored, audience_size
    ```

    - `names_resolved = 0` → **your item names are not in the corpus.** This is a vocabulary problem, not a data absence. Go back to `bjl_items`, find the real names, and re-run the arm. Do not report "no audience affinity."
    - `names_resolved > 0` but `audience_size < 30` → the audience is genuinely too small to say anything. Report that plainly, with the number.
    - `audience_size >= 30` but the arm still returned nothing → there is genuinely no distinctive affinity clearing the score floor. This is the one case where "no distinctive preference" is the honest finding, and it is worth stating.

    Log the `bjl_audience_size` row in scratch as a `type: "meta"` note so the synthesizer can see which of the three it was. **Never write "no audience affinity found" off a bare zero-row result** — on every zero-row call measured to date the real cause was unresolved names or an audience below the floor, and reporting those as an absence states something about the population that the data never said.
  - **Do not run `bjl_audience_size` before the arm.** It costs about as much as the arm itself, so a pre-flight check slows down every call that was going to succeed. Run it only on a zero-row result, where it is the difference between a diagnosis and a shrug.

...
**Addendum 6 · Security posture the build established (context for future functions).** Two default-privilege flips make new objects fail-closed: `bjl_agent_readonly` and `anon`/`authenticated` no longer inherit EXECUTE on functions created by `postgres` in `public`. Every future agent- or public-facing function therefore needs a **deliberate** GRANT (this is why the corpus-arm migration re-grants explicitly after its DROP/CREATE). One live critical was closed in the same pass: `execute_write_sql` (SECURITY DEFINER, arbitrary SQL) had been `anon`-executable and is now service-role only. Open on the ledger: the analogous **table** and **sequence** defaults still grant to `anon`/`authenticated`; RLS covers existing tables but a future table created without RLS would be born exposed — flagged, not yet flipped, pending a read of how the frontend uses table-level PostgREST. [SUPERSEDED → see addendum 8]

**Addendum 8 · The table defaults were not theory (closes addendum 6's open item).** A third-party audit the same evening proved the flagged exposure live: `anon` and `authenticated` held INSERT/UPDATE/DELETE on ~70 tables — the whole public schema, including `bjl_responses` (2.2M rows), `bjl_respondents` (PII), and `bjl_authorized_users`/`bjl_approved_emails`, the access whitelists themselves. Since `anon` ships in the client JS, this was a public arbitrary-write path, and addendum 6's "currently theory" was wrong. **Write is closed** schema-wide plus fail-closed defaults for future tables and sequences; verified `tables_with_anon_write_remaining = 0`. **Read is deliberately still open**, closing per-table rather than blanket, because a wrong SELECT revoke darkens the public tool. The frontend read addendum 6 was waiting on is done and came back decisive: **this frontend does not use table-level PostgREST at all** — `index.html` builds one anon client and calls only `sb.auth.*` on it (zero `.from()`, zero `.rpc()`), `public-chat.html` touches Supabase not at all, and all 37 function clients are service_role except one anon client used solely for `auth.getUser()`. The anon key is an authentication credential here, not a data credential. The remaining caveat is out-of-repo callers, which no grep can see. Applied: `2026-08-04_revoke_anon_write_schemawide.sql`.
